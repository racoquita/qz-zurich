var App = function() {
	...
	this.on = function() {
		if(!this.isMobile()) {
			$('.cta').css('pointer-events', 'auto').on("click", function(e) {
				e.preventDefault();
				videoPlayer.play();
			});
		} else {
	...
	this.off = function() {
		...
		$('.cta img:first-child').attr('src', 'http://ads.qz.com/sponsors/zurich/love/2014-05-15/mobile/images/cta.png');
	}
	this.playVideo = function() {
		brightcove.createExperiences();
	}
	this.stopVideo = function() {
		$('.video').css('z-index', 101);
    	$('.video-container').css('z-index', 1);

    	if(that.isAndroid()) {
			$('.video').css('pointer-events', 'auto').on("click", function() {
				videoPlayer.play();
			});
		}
	}
	...
	window.onTemplateReady = function ( e ) {
	    ...
	    $('.BrightcoveExperience').on("click", function() {
			videoPlayer.play();
		});
	    ...
	    videoPlayer.addEventListener(brightcove.api.events.MediaEvent.COMPLETE, function(){
			...
			$('.cta img:first-child').attr('src', 'http://ads.qz.com/sponsors/zurich/love/2014-05-15/mobile/images/learn-more.png');
	    	that.stopVideo();
	    });
